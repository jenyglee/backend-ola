...
public class RecommendCourseLikeServiceImpl implements RecommendCourseLikeService{
    ...
    public void likeRecommendCourse(Long id, User user){
        ...
        Optional<CourseLike> findByUserEmail = likeRecommendRepository.findByUserEmailAndBoard(user.getEmail(),recommendCourseBoard);
    ...
    public void unLikeRecommendCourse(Long id, User user){
        ...
        CourseLike findByUserEmail = likeRecommendRepository.findByUserEmailAndBoard(user.getEmail(),recommendCourseBoard).orElseThrow(()->new CustomException(Status.CONFLICT_LIKE));
        //레파지토리에서 좋아요를 삭제한다.
